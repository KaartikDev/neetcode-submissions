class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        for i in range(1,len(nums)):
            if i == len(nums)-1:
                output[i] = running #running does not inclde index we at 
            else:
                output[i] = running*rightProducts[i] 
            running*=nums[i] #running now includes index we at and everything to left
